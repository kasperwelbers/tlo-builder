...
app.post("/api/auth/verify-code", async (c) => {
  ...
  const { n } = (await c.env.DB.prepare(
    "SELECT COUNT(*) as n FROM user_roles WHERE project_id = '' AND role = 'admin'"
  ).first<{ n: number }>()) ?? { n: 0 }
  if (n === 0) {
    await c.env.DB.prepare(
      "INSERT OR IGNORE INTO user_roles (user_id, role, project_id) VALUES (?, 'admin', '')"
    )
      .bind(row.id)
      .run()
  }
...
app.post("/api/projects", async (c) => {
  const user = await requireSession(c)
  if (!user) return c.json({ error: "Unauthorized" }, 401)
  if (!(await isServerAdmin(user.userId, c.env)))
...
app.patch("/api/projects/:id", async (c) => {
  ...
  if (!(await canAccessProject(user.userId, projectId, c.env)))
    return c.json({ error: "Forbidden" }, 403)
...
app.post("/api/projects/:id/members", async (c) => {
  ...
  let target = await c.env.DB.prepare("SELECT id FROM users WHERE email = ?")
    .bind(normalised)
    .first<{ id: string }>()
  if (!target) {
    // User hasn't logged in yet — pre-create their account so the role can be
    // granted now. When they sign in via OTP the verify-code route will find
    // this existing row and reuse it.
    const newId = crypto.randomUUID()
    await c.env.DB.prepare(
      "INSERT INTO users (id, email, created_at) VALUES (?, ?, ?)"
    )
      .bind(newId, normalised, Date.now())
      .run()
    target = { id: newId }
  }
  await c.env.DB.prepare(
...
app.delete("/api/projects/:id/members/:userId", async (c) => {
  ...
  await c.env.DB.prepare(
    "DELETE FROM user_roles WHERE user_id = ? AND project_id = ? AND role = 'admin'"
  )
...
app.get("/api/admin/users", async (c) => {
  ...
  const rows = await c.env.DB.prepare(
    ...
    .then((r) => r.results)
  return c.json(
    rows.map((r) => ({ ...r, is_server_admin: !!r.is_server_admin }))
  )
})
...
app.delete("/api/admin/server-roles/:userId", async (c) => {
  ...
  if (targetId === user.userId)
    return c.json({ error: "You can't remove your own admin" }, 400)
  await c.env.DB.prepare(
    "DELETE FROM user_roles WHERE user_id = ? AND role = 'admin' AND project_id = ''"
  )
    .bind(targetId)
    .run()
  return c.json({ ok: true })
...
app.get("/ws/:projectId", async (c) => {
  ...
  await c.env.DB.prepare("DELETE FROM ws_tickets WHERE ticket = ?")
    .bind(ticket)
    .run()
