...
export async function sendOtp(email: string, env: Env): Promise<void> {
  ...
  const existing = await env.DB.prepare(
    "SELECT expires_at FROM otp_codes WHERE email = ?"
  )
    .bind(email)
  ...
  // Send the email best-effort. In local dev the send_email binding with
  // remote = true can be unavailable or crash the worker before a response
  // is sent — the console log above is sufficient for development login.
  try {
  ...
  `,
    ...
    console.error(
      "[Auth] Email send failed (code is logged above for dev use):",
      emailErr
    )
  }
}
